...
<head>
    ...
    <title>Guru dan Staff | TK Pertiwi Grojogan</title>
    <style>
        .teacher-card.hidden {
            display: none;
        }
...
<body class="bg-gray-100">
    ...
    <section class="guru min-h-screen overflow-hidden pt-10" data-aos="fade-up">
        <div class="max-w-7xl mx-auto text-center pt-8 px-4">
            <h2 class="font-bold text-[20px] md:text-[32px] text-[#0090D4] flex items-center justify-center text-center"
                ...
                GURU & STAFF</h2>
            <p class="font-regular text-sm md:text-[18px] text-black mb-10" data-aos="fade-up" data-aos-delay="200">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-4" id="teachers-container">
                @foreach($teachers as $key => $teacher)
                    <div class="teacher-card {{ $key >= 8 ? 'hidden' : '' }} bg-white shadow-lg rounded-2xl overflow-hidden flex flex-col"
                        data-aos="fade-up" data-aos-delay="{{ 300 + ($key % 4) * 100 }}">
                        <div class="w-full px-4 pt-4 pb-2">
                            <div class="aspect-[3/4] w-full">
                                @if($teacher->photo_path)
                                    <img src="{{ asset($teacher->photo_path) }}" alt="{{ $teacher->name }}"
                                        class="w-full h-full object-cover rounded-lg border-b-3 border-[#0090D4]" loading="lazy">
                                @else
                                    <div
