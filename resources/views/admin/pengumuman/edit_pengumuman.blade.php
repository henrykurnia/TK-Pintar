...
<head>
    ...
    <title>Edit Pengumuman | Admin TK Pintar</title>
</head>
    ...
    <div id="root">
        <div class="relative md:ml-64 bg-blueGray-50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 items-center md:px-10 px-4 mt-4">
                <a class="text-black text-sm uppercase font-semibold lg:inline-block">Edit Pengumuman</a>
                @include('admin.partial.user_info')
            </div>

            <!-- FORM CONTAINER -->
            <div class="p-6">
                <form action="{{ route('pengumuman.update', $announcement->id) }}" method="POST" class="space-y-6 bg-white p-6 rounded-xl shadow-md">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea id="content" name="content" rows="6" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:border-blue-300">{{ $announcement->content }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="date" id="date" name="date" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:border-blue-300"
                                value="{{ \Carbon\Carbon::parse($announcement->date)->format('d M Y') }}" />
                        </div>
